...
MedicalFile::MedicalFile(vector<string> newCurrentMedications, string newBloodType, vector<string> newAllergies, vector<string> newVaccinationRegistry, vector<string> newNotes, string newSex) 
: currentMedications(newCurrentMedications), bloodType(newBloodType), sex(newSex), allergies(newAllergies), vaccinationRegistry(newVaccinationRegistry), notes(newNotes) {}
...
string MedicalFile::getBloodType() {   
    return bloodType;
}

vector<string> MedicalFile::getAllergies() {
    return allergies;
}
...
vector<string> MedicalFile::getNotes() {
    return notes;
}
