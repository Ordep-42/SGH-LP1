...
ScheduledAppointments Schedule::getScheduledAppointments() {
...
bool isInSession(Session *session, WorkSchedule sessions) {

    for (Session sessionC : sessions.getWorkSchedule()) {
        if (session->isEqualTo(sessionC)) {
            return true;
        }
    }
    return false;
...
bool Schedule::checkAvaiability(Session checkMe) {
    if (isInSession(&checkMe, this->getWorkSchedule())) {
        return true;
    }
    return false;
}
