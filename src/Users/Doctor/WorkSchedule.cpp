...
// ----------------------------------------------------------------------------------------------------
// CONSTRUCTOR'S: (Essa forma assegurará que seções iguais não serão adicionadas
// no vetor.)
// ----------------------------------------------------------------------------------------------------
WorkSchedule::WorkSchedule(/*vector<WorkSession> newSchedule,*/ int newId,
                           int newDoctor_id)
...
bool WorkSchedule::testSession(
    Session testSession) { // por conta disso, esse será bool mesmo! Mas
                           // poderíamos manter o padrão pra coletar erros.
    for (WorkSession wSession : schedule) {
        if (testSession.isEqualTo(wSession)) {
            return true;
        }
    }
    return false;
...
WorkSchedule WorkSchedule::searchByTimeBetween(
    Time time1, Time time2) { // tratar pra ver se veio vazio ou se não.
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
...
WorkSchedule WorkSchedule::searchByTimeBefore(
    Time thatTime) { // tratar pra ver se veio vazio ou se não.
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
        if ((wSession.getTime().isBeforeThan(thatTime)) ||
            thatTime.isEqualTo(wSession.getTime())) {
            vetorWS.push_back(wSession);
        }
    }
    WorkSchedule wScheduleBtwn(vetorWS, id, doctor_id);
...
WorkSchedule WorkSchedule::searchByTimeAfter(
    Time thatTime) { // tratar pra ver se veio vazio ou se não.
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
        if ((thatTime.isBeforeThan(wSession.getTime())) ||
            thatTime.isEqualTo(wSession.getTime())) {
            vetorWS.push_back(wSession);
        }
    }
...
WorkSchedule WorkSchedule::searchByDateBetween(
    Date date1, Date date2) { // tratar pra ver se veio vazio ou se não.
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
...
WorkSchedule WorkSchedule::searchByDateBefore(
    Date thatDate) { // tratar pra ver se veio vazio ou se não.
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
        if ((wSession.getDate().isBeforeThan(thatDate)) ||
            thatDate.isEqualTo(wSession.getDate())) {
            vetorWS.push_back(wSession);
        }
    }
    WorkSchedule wScheduleBtwn(vetorWS, id, doctor_id);
...
WorkSchedule WorkSchedule::searchByDateAfter(
    Date thatDate) { // tratar pra ver se veio vazio ou se não.
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
        if ((thatDate.isBeforeThan(wSession.getDate())) ||
            thatDate.isEqualTo(wSession.getDate())) {
            vetorWS.push_back(wSession);
        }
    }
...
WorkSchedule WorkSchedule::searchByID(
    int testID) { // Caso o retorno tenha size 0, o id era invalido, caso tenha
                  // tamanho 1, era valido, caso tenha >1, erro!
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
...
// BRUTEFORCE REMOVEWS:
WorkSchedule
WorkSchedule::removeWS(WorkSession removeMe) { // remve uma workSession do vetor
    vector<WorkSession> vetorWS;
    for (WorkSession wSession : schedule) {
        if (not removeMe.isEqualTo(wSession)) {
            vetorWS.push_back(wSession);
        }
    }
...
void WorkSchedule::safeAdd(WorkSession addMe) {
    if (schedule.size() == 0) {
        schedule.push_back(addMe);
    } else {
        bool isIn = false;
        for (WorkSession wSession : schedule) {
            if (wSession.isEqualTo(addMe)) {
                isIn = true;
            }
        }
        if (not isIn) {
