...
vector<Appointment> Schedule::searchByDate() { return scheduledAppointments; }

vector<Appointment> Schedule::searchByStatus() { return scheduledAppointments; }

bool sectionEquals(Section section1, Section section2) {
    if (section1.date.isEqualTo(section2.date) &&
        section1.time.isEqualTo(section2.time)) {
        return true;
    } else {
        return false;
    }
...
bool sectionIsBeforethan(Section section1, Section section2) {
    if (section1.date.isBeforeThan(section2.date)) {
        return true;
    }
    if (section1.date.isEqualTo(section2.date) &&
        section1.time.isBeforeThan(section2.time)) {
        return true;
    }
    return false;
}
...
bool Schedule::isAppointed(Section test) { // nome meme, pse...
    for (Appointment appointment : scheduledAppointments) {
        if (sectionEquals(appointment.section, test)) {
            return true;
        } // Falta verificar o status da consulta!!!
    }
...
Section Schedule::nextWorkSection() { // o mais recente em tempo
    ...
    sectionC.date = workSchedule[0].date;
    sectionC.time = workSchedule[0].time;
    for (Section section : workSchedule) {
        if (sectionIsBeforethan(section, sectionC)) {
            sectionC.date = section.date;
            sectionC.time = section.time;
        }
    }
...
Appointment Schedule::nextAppointment() {
    ...
    for (Appointment appointment : scheduledAppointments) {
        if (sectionIsBeforethan(
                appointment.section,
                appointmentC.section)) { // FALTA VERIFICAR O STATUS!!!!
...
Section nextSection(vector<Section> sections) { // o mais recente em tempo
    ...
    sectionC.date = sections[0].date;
    sectionC.time = sections[0].time;
...
vector<Section> removeSection(vector<Section> sections,
