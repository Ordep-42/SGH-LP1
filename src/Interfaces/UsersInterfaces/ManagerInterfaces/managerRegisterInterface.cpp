...
void HospitalInterface::managerRegisterInterface() {
    ...
    Schedule doctorSchedule =
        HospitalDatabase::getScheduleByDoctorID(doctors[choice - 1]);
    ...
    if (doctorWorkSessions.getWorkSchedule().size() == 0) {
        ...
        if (choicce == 1) {
            ...
            HospitalInterface::managerRegisterInterface2(doctors[choice - 1],
                                                         mes, ano, dia);
        } else {
            string date, time;
...
void HospitalInterface::managerRegisterInterface2(int doctor_id, int month,
                                                  int year, int day) {
    // coloque um mes de 30 dias apenas
    for (int i = day; i <= 5; i++) {
        Time time(7, 0, 0);
        Date date(i, month, year);
        Time sumMe(1, 0, 0);
        Session secao(time, date);
        HospitalDatabase::createWSession(doctor_id, secao);
        time.sumTime(sumMe);
        Session secao2(time, date);
        HospitalDatabase::createWSession(doctor_id, secao2);
        time.sumTime(sumMe);
        Session secao3(time, date);
        HospitalDatabase::createWSession(doctor_id, secao3);
        time.sumTime(sumMe);
        Session secao4(time, date);
        HospitalDatabase::createWSession(doctor_id, secao4);
        time.sumTime(sumMe);
        Session secao5(time, date);
        HospitalDatabase::createWSession(doctor_id, secao5);
        time.sumTime(sumMe);
        Session secao6(time, date);
        HospitalDatabase::createWSession(doctor_id, secao6);
    }
    cout << " Cadastro realizado com sucesso " << endl;
    cout << "pressione enter /p continuar" << endl;
    getchar();
